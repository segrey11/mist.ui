...
const expectedResourceCounters = {};
Polymer({
  is: 'mist-socket',
  url: '/socket',
  properties: {
  ...
  _initializeexpectedResourceCounters(clouds) {
    // If the resource types increase, we should probably store them in an array and iterate over them

    expectedResourceCounters.machines = clouds.filter(
      cloud => cloud.enabled
    ).length;
    expectedResourceCounters.zones = clouds.filter(
      cloud => cloud.dns_enabled
    ).length;
    // TODO: Set counters for volumes, networks and images
  },
  /* eslint-enable no-param-reassign */
  _updateClouds(data) {
    ...
    if (!data.length) {
      ...
      this._initializeexpectedResourceCounters(data);
  ...
  _updateMachines(data) {
    expectedResourceCounters.machines -= 1;
    if (expectedResourceCounters.machines <= 0) {
      this.set('model.onboarding.isLoadingMachines', false);
    }
